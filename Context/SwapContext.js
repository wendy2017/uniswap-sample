...
const SwapTokenContextProvider = ({ children }) => {
  ...
  const fetchingData = async () => {
    try {
      ...
      console.log("balance==", convertedBigInt);
      ...
      let tokensTemp = [];
      console.log("addToken==", connection, addToken, userAccount);
      addToken.map(async (_token, idx) => {
        ...
        console.log("tokensTemp : ", userBalance, convertedTokenBalance);
      ...
      console.log("tokenlist==", axiosData.data.data.tokens);
  ...
  const singleSwapToken = async ({ token1, token2, swapAmount }) => {
    console.log("token 1: ", token1, token1.address);
    console.log("token 2: ", token2, token2.address);
    ...
    try {
      ...
      weth9Contract = await connectingWithIWTHToken();
